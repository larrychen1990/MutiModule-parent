...
public class CaptchaUtil {
	...
	public BufferedImage genRandomCodeImage(StringBuffer randomCode) {
		...
		for (int i = 1; i <= randomCode.length(); i++) {
			drowString(g, i, randomCode);
		}
		g.dispose();
		return image;
	}
	 ...
	 * 绘制字符串
	 */
	private String drowString(Graphics g, int i, StringBuffer randomCode) {
		g.setColor(new Color(random.nextInt(101), random.nextInt(111), random
				.nextInt(121)));
		String rand = randomCode.charAt(i-1)+"";
