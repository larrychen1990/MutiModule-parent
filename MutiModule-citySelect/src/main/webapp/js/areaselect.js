...
(function ($, undefined) {
    $.widget("ui.areaselect", {
        ...
        showAddress: function (address, jId) {//设置input中显示的值
            ...
            $(jId).val(nlist.join("-")).attr("code", clist.join("-"));
            ///该处为赋值处
            var txtValue = $(jId).val();
            if (txtValue.split('-').length > 1) {
                var txtCode = $(jId).attr("code");
                var cityId = txtCode.split('-')[1];
                var strType = "PickUp";
                var strIsCheckedid = "IsPickUp";
                var strSelectId = "PickUpArea";
                if (jId.indexOf("source") == -1) {
                    strType = "Delivery";
                    var strIsCheckedid = "IsDelivery";
                    var strSelectId = "DeliveryArea";
                }
                var areaId = "";
                var area = { isSuccess: true, errMsg: "", code: "" };
                if (txtValue.split('-').length > 2) {
                    areaId = txtCode.split('-')[2];
                    //pdAreaChange(strIsCheckedid, strSelectId, $(jId).attr("id"));
                }
                function BindAreaCallBack(jsonStr) {
                    if (jId.indexOf("source") > -1) {
                        $("#divPickUp").html(jsonStr);
                        $("#PickUpTown").html("");
                        $("#PickUpTown").hide();
                    }
                    else {
                        $("#divDelivery").html(jsonStr);
                        $("#DeliveryTown").html("");
                        $("#DeliveryTown").hide();
                    }
                    if (txtValue.split('-').length > 2) {
                ...
                //AjaxJsonSubmit('/Public/BindArea', { CityId: cityId, AreaId: areaId, StrType: strType }, null, BindAreaCallBack);
            }
        },
        ...
        _initSource: function () {
            ...
            if (typeof this.options.source === "string") {
            	...
            	//ie6用此方法转换json对象最快
                this.source = this.options.source = eval("(" + this.options.source + ")"); 
                return true;
            }
        ...
        */
        ...
        initTab: function (address) {//根据address值设置tab
            var adds = (typeof address != "object") ? this.address : address,i,
        ...
        _create: function () {//创建dom框架，初始化
            ...
            if (3 == grade) {
				...
				<span class="alp-close alp-close-area"><img class="icon_Close" src="image/tabs_close.gif" alt="close" /></span>\
				<div class="c"></div>\
			</ul>\
